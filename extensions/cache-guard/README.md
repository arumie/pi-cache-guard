...
It listens for finalized assistant messages, reads their provider usage (`input`, `cacheRead`, `cacheWrite`, and usage cost), and estimates the uncached portion of the shared prompt prefix between adjacent requests. It only begins evaluating misses after cache activity has been observed, avoiding false positives for models and providers that do not expose prompt caching. Actual-only mode is enabled by default, so significant misses after a model switch or a five-minute-plus idle gap are ignored and only unexplained cache misses update the guard counters.
...
The `/cache-guard` command stores configuration under `~/.pi/agent/cache-guard-settings.json`; the extension reloads those settings at every session start. Use `/cache-guard actual-only off` to restore the original behavior where model-switch and idle-related misses count. The command uses these subcommands:
...
/cache-guard reset
/cache-guard actual-only on|off
